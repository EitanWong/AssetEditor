    ...
    public class AssetTree
    {
        ...
        public void AddAsset(string guid)
        {
            ...
            while (startIndex < length)
            {
                ...
                AssetData pathNode = new AssetData(endIndex == -1 ? guid : null, directory,
                    assetPath.Substring(0, endIndex == -1 ? length : endIndex), node.Level == 0);
        ...
        public TreeNode<AssetData> FindAssetByGuid(string guid)
        {
            ...
            while (startIndex < length)
            {
                ...
                string directory = assetPath.Substring(startIndex, subLength);
                //Debug.Log(directory);
                var match= node.Children.Find(node => node.Data.path.Equals(directory));
                if (match != null)
                    node = match;
                // AssetData pathNode = new AssetData(endIndex == -1 ? guid : null, directory,
                //     assetPath.Substring(0, endIndex == -1 ? length : endIndex), node.Level == 0);
